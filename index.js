...
/*const reply = (teks) => {
			xeon.sendMessage(from, teks, text, { thumbnail: iye, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jam} - ${week} ${weton} - ${date}`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/6282221792667?text=hello`}}})
		}*/
...
const fakewa = (teks) => {        
xeon.sendMessage(from, teks, text, { thumbnail: fake, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `Xeon`,body:"",previewType:"PHOTO",thumbnail:iye,sourceUrl:`https://wa.me/6282221792667?text=hello`}}})
		}
...
"mediaType": "2",
"thumbnailUrl": "https://i.ibb.co/y0RYgzB/FB-IMG-1635413002830.jpg",
"mediaUrl": "https://youtube.com/cnnel/UCvAo9TZ0Pw9vrJ_0WYRyO3A",
...
*「 𝐃𝐨𝐠𝐞 𝐁𝐨𝐭 」*`
         haitod = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${_registered.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
             buttons = [{buttonId: `menu`,buttonText:{displayText: `</️MENU`},type:1}]
              ...
              buttonsMessage = {footerText:'SALAM DARI TASIKMALAYA', imageMessage: imageMsg,
...
break
case 'okeyx':
reply('SELAMAT DATANG SEMOGA BETAH YAH.<3')
break
...
case 'mygithub':
  reply('https://github.com/BOTCAHX')
					break;
   case 'ss':
      reply(mess.wait)
...
- *Link Dev* : ${i.link_dev}\n❉─────────────────────❉`;
      ...
      case "on":
        if (!mek.key.fromMe) return;
        offline = false;
        fakestatus(" ```YOU ARE ONLINE BY BY OWN BOTCAHBOTz``` ");
        break;
      case "status":
        fakestatus(
...
*[Wait] Wait a moment..*`
      ...
      case "s":
        ...
        break;
      case "toimg":
        if (!isQuotedSticker) return reply("𝗥𝗲𝗽𝗹𝘆/𝘁𝗮𝗴 STICKER !");
        reply(mess.wait);
        encmedia = JSON.parse(JSON.stringify(mek).replace("quotedM", "m"))
          .message.extendedTextMessage.contextInfo;
        media = await xeon.downloadAndSaveMediaMessage(encmedia);
